...
# =============================
# Translation: better Google Persian + optional Groq
# =============================
...
# اصطلاحاتی که بهتر است در فضای کریپتو انگلیسی بمانند.
CRYPTO_TERMS = [
...
def init_translator() -> None:
    """Initialize translation engine. Default is improved Google translation."""
    ...
    # Optional: Groq only if explicitly requested or TRANSLATE_ENGINE=auto
...
def normalize_tweet_text(text: str) -> str:
    """Clean HTML/noisy RSS text before translation and filtering."""
...
def protect_special_terms(text: str) -> Tuple[str, List[Tuple[str, str]]]:
    """Keep URLs, @users, #hashtags, $tickers and crypto terms unchanged."""
...
def postprocess_persian_translation(text: str) -> str:
    """Make Google output a bit more natural for Persian crypto Twitter."""
    replacements = {
        "قطره هوایی": "airdrop",
        "قطره‌های هوایی": "airdrops",
        "ایردراپ": "airdrop",
        "ایردراپ‌ها": "airdrops",
        "شبکه اصلی": "mainnet",
        "شبکه آزمایشی": "testnet",
        "فهرست شدن": "لیست شدن",
        "فهرست شده": "لیست شده",
        ...
        "نشانه": "توکن",
        "نشانه‌ها": "توکن‌ها",
        "ادعا کنید": "claim کنید",
        "مطالبه کنید": "claim کنید",
        "راه اندازی": "لانچ",
        "راه‌اندازی": "لانچ",
        "صعودی": "bullish",
        "نزولی": "bearish",
...
def translate_fa(text: str) -> Optional[str]:
    ...
    # اگر متن عمدتاً فارسی است، ترجمه نکن.
...
def default_filters() -> Dict[str, Any]:
    return {
        "global": {"filter_rt": True, "filter_replies": True},
        "chats": {},
    }
...
def clean_username(raw: str) -> str:
    ...
    for domain in ("x.com/", "twitter.com/", "nitter.net/", "nitter.poast.org/", "xcancel.com/"):
        if domain in raw.lower():
            # حفظ کیس اصلی بعد از دامنه مهم نیست چون یوزرنیم را پایین‌تر lower می‌کنیم.
...
def should_send(chat_id: Any, username: str, text: str) -> Tuple[bool, str, bool]:
    ...
    # Alert bypasses RT/reply/keyword filters.
...
def build_tweet_message(username: str, title: str, fa_text: Optional[str], is_alert: bool) -> str:
    ...
    if fa_text and fa_text.strip() != title.strip():
        ...
        # پرچم شیر و خورشید: یونیکد رسمی ندارد، پس از ترکیب شیر + خورشید استفاده شده.
...
async def send_tweet_entry(chat_id: Any, username: str, entry: Any, bot: Any) -> Tuple[bool, str]:
    ...
    try:
        if image_url and image_url.startswith("http") and len(text) <= 1024:
            ...
            # Caption limit is 1024. For longer translated messages, send photo first, then full text.
            try:
                await bot.send_photo(chat_id=chat_id, photo=image_url, disable_notification=disable_notification)
            except Exception as e:
                logger.warning(f"Photo send skipped, will send text only: {e}")
    ...
    while True:
        ...
        for username, info in list(tracked.items()):
            try:
                ...
                # اگر last_id در فید پیدا نشد، برای جلوگیری از اسپم فقط چند مورد آخر ارسال شود.
                ...
                for entry in reversed(new_entries):
                    ...
                    if username in tracked:
                        tracked[username]["last_id"] = str(tweet_id)
                        save_tracked()
...
async def cmd_filter(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) >= 2:
        ...
        on = parse_on_off(val)
        if on is None:
            await update.message.reply_text("مقدار باید on یا off باشد. مثال: /filter rt on")
            return
...
async def cmd_keywords(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        ...
        await update.message.reply_text(
            "کلمات کلیدی: "
            + (", ".join(kws) if kws else "هیچی - همه ارسال میشه")
...
async def check_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for username in usernames:
        ...
        for entry in reversed(feed.entries[:count]):
            ok, _reason = await send_tweet_entry(chat_id, username, entry, context.bot)
            if ok:
                sent += 1
            else:
                filtered_count += 1
...
async def cmd_export(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send DB files as backup. Restricted if ADMIN_CHAT_ID is configured."""
    if not update.effective_chat or not update.message:
        return
    ...
    try:
        ...
        await update.message.reply_text("✅ بکاپ ارسال شد.\nبرای بازگردانی، همین ۲ فایل را دوباره به ربات بفرست.")
    except Exception as e:
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Validate JSON before replacing the real database.
