...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    msg = (
        f"👋 سلام! به ربات مانیتورینگ X خوش آمدید.\n\n"
        f"📊 **وضعیت کنونی این چت:**\n"
        f"🔹 تعداد اکانت‌های تحت نظر: `{accounts_count}`\n"
        f"🔹 موتور ترجمه فعال: `{get_translate_status()}`\n"
        f"🔹 فیلتر ریتوییت‌ها: `{'روشن ✅' if cf.get('filter_rt', True) else 'خاموش ❌'}`\n"
        f"🔹 فیلتر ریپلای‌ها: `{'روشن ✅' if cf.get('filter_replies', True) else 'خاموش ❌'}`"
    )
...
# =============================
# Background Tasks (حذف لوپ رگباری بکاپ)
# =============================
async def auto_backup(app: Application) -> None:
    ...
    # مکث اولیه موقع لود پروژه جهت جلوگیری از پیام ناگهانی
    ...
    while True:
        ...
        # مکس دقیقاً طبق عدد ست شده در پروژه (بدون فرمول محدود کننده min)
...
async def post_init(application: Application) -> None:
    # ثبت تسک‌های پس‌زمینه به درستی پس از لود کامل توابع بالا
    asyncio.create_task(check_twitter_updates(application))
    asyncio.create_task(auto_backup(application))
    
    # تنظیم منوی دکمه‌های ربات تلگرام
    commands = [
        BotCommand("start", "شروع کار با ربات و وضعیت چت"),
        BotCommand("stats", "آمار وضعیت فنی سرور ربات"),
        BotCommand("export", "دریافت فایل پشتیبان دستی دیتابیس اکانت‌ها"),
    ]
...
def main() -> None:
    ...
    # اتصال دستورات به توابع تعریف شده بالادستی
    application.add_handler(CommandHandler("start",     start))
    application.add_handler(CommandHandler("stats",     cmd_stats))
    application.add_handler(CommandHandler("export",    cmd_export))

    logger.info("Bot infrastructure fully initialized. Starting polling...")
