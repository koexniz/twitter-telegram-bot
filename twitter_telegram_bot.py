    ...
    while True:
        ...
        for username, info in list(tracked.items()):
            try:
                ...
                # ── مهم: همه tweet_id های جدید رو جمع کن ──
                # بعد از پردازش، بزرگ‌ترین (جدیدترین) ID رو به‌عنوان last_id ذخیره می‌کنیم
                # حتی اگه ارسال با خطا شکست خورد، ID رو ثبت می‌کنیم تا تکرار نشه
                ...
                for entry in reversed(new_entries):
                    ...
                    for chat_id in list(info.get("chats", [])):
                        # dedup چک — اگه قبلاً ارسال شده skip
                    ...
                    # این کلیدی‌ترین تغییر است: حتی اگه ارسال fail شد،
                    # ID رو mark می‌کنیم تا دیگه تکرار نشه
                    processed_ids.append(tid)

                # آپدیت last_id به جدیدترین توییت پردازش‌شده
                if processed_ids and username in tracked:
                    # آخرین عنصر لیست = جدیدترین توییت (چون reversed کردیم)
