...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"ترجمه: {get_translate_status()}\n"
        f"اکانت‌ها: {accounts_count}\n"
        f"فیلتر RT: {'✅' if cf.get('filter_rt') else '❌'} | "
        f"ریپلای: {'✅' if cf.get('filter_replies') else '❌'}\n"
        f"آلارم: {len(cf.get('alert_keywords', []))} کلمه\n\n"
        "/add user1 user2 — اضافه کردن\n"
        "/remove user — حذف\n"
        "/list — لیست اکانت‌ها\n"
        "/check [user] [count] — چک دستی\n"
        "/filter rt on|off\n"
        "/filter replies on|off\n"
        "/keywords add|list|clear\n"
        "/alert add|list|clear\n"
