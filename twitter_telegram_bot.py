...
def extract_tweet_id(entry: Any) -> str:
    """استخراج tweet_id از چند منبع مختلف برای مقاوم بودن در برابر تفاوت RSS sourceها."""
    # ۱. از لینک مستقیم
    ...
    # ۴. از description (بعضی RSS sourceها لینک رو داخل description میذارن)
...
async def send_tweet_entry(chat_id: Any, username: str, entry: Any, bot: Any) -> Tuple[bool, str]:
    ...
    # ── ثبت dedup بعد از ارسال (موفق یا ناموفق با پیام) ──
    mark_as_sent(chat_id, tweet_id)
...
async def auto_backup(app: Application) -> None:
    """هر BACKUP_INTERVAL ثانیه یه بار بکاپ خودکار به ادمین میفرسته."""
    await asyncio.sleep(60)  # یه دقیقه بعد از استارت شروع کن
    while True:
        try:
            if ADMIN_CHAT_ID:
                caption_map = {
                ...
                logger.warning("Auto-backup: ADMIN_CHAT_ID not set, skipping")
        except Exception as e:
    ...
    while True:
        ...
        for username, info in list(tracked.items()):
            try:
                ...
                # اگه last_id پیدا نشد:
                # - dedup کافیه که از تکرار جلوگیری کنه
                # - ولی برای جلوگیری از اسپم اولیه، محدود می‌کنیم
                if last_id and not found_last_id:
                    # فیلتر کن entries هایی که dedup قبلاً دیده
                    # (ممکنه RSS source عوض شده و ID ها کمی فرق دارن)
                    chats_of_user = list(info.get("chats", []))
                    truly_new = []
                    for e in new_entries:
                        tid = extract_tweet_id(e)
                        # اگه هیچ چتی این ID رو ندیده، واقعاً جدیده
                        if not any(is_already_sent(c, tid) for c in chats_of_user):
                            truly_new.append(e)
                    # اگه بیشتر از حد مجاز بود، محدود کن
                ...
                for entry in reversed(new_entries):
                    ...
                    for chat_id in list(info.get("chats", [])):
                        # dedup چک — اگه قبلاً دیده شده skip
                        if is_already_sent(chat_id, tid):
                            logger.debug(f"[DEDUP] skip {username}/{tid} → {chat_id}")
                            continue
                        try:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif data == "cmd_check_all":
        # چک همه اکانت‌های این چت
...
async def check_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_chat:
        return
    # پشتیبانی از هر دو حالت: message و callback_query
    chat_id = update.effective_chat.id
